...
const SellerWidgetList = ({ seller, rating, price, sellerId }) => {
  return (
    <div className="container flex flex-col mx-8 p-2 border border-black/20 rounded-md gap-2">
      <div className="text-sm text-blue-500 font-semibold p-1 bg-blue-100 rounded-sm">
        <Link to={`/seller/${sellerId}`}>{seller}</Link>
        <span className="mx-1 bg-green-500 rounded-md text-xs text-white px-4">
          {rating}
        </span>
      </div>
      <div className="flex items-center p-1 text-sm text-gray-500 ">
        ...
        <span className="mx-2">Takip Et</span>
      </div>
      <div className="flex items-center p-1 text-sm text-gray-500 ">
        <span className="hover:text-orange-600 bg-gray-400 p-1 rounded-sm text-black">
          <ChatIcon />
        </span>
        <span className="mx-2">Satıcıya Soru Sor</span>
      </div>
